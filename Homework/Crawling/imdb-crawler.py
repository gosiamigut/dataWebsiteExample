...
from pattern.web import Crawler, DEPTH, BREADTH, FIFO, LIFO ### ADDED LINE
...
def scrape_movie_page(dom):
    ...
    # Will scrape everything of interest in "div.title-overview".
    e = dom.by_tag("div.title-overview")[0]
    # For example, finds "title" by selecting the first "h1.header" and then the first element with the attribute "itemprop='name'" therein.
    title = [t.content for t in e.by_tag("h1.header")[0].by_attr(itemprop="name")][0]
    duration = [d.content.split()[0] for d in e.by_tag("div.infobar")[0].by_attr(itemprop="duration")][0]
    genres = ";".join([g.content for g in e.by_tag("div.infobar")[0].by_attr(itemprop="genre")])
    directors = ";".join([d.content for d in e.by_attr(itemprop="director")[0].by_attr(itemprop="name")])
    writers = ";".join([c.content for c in e.by_attr(itemprop="creator")[0].by_attr(itemprop="name")])
    actors = ";".join([a.content for a in e.by_attr(itemprop="actors")[0].by_attr(itemprop="name")])
    rating = [r.content for r in e.by_tag("div.star-box-details")[0].by_attr(itemprop="ratingValue")][0]
    n_ratings = [n_r.content.replace(",", "") for n_r in e.by_tag("div.star-box-details")[0].by_attr(itemprop="ratingCount")][0]
